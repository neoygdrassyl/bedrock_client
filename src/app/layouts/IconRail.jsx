      ...
      <TooltipTrigger asChild>
        <button
          ...
          className={cn(
            ...
            isActive
              ? 'bg-sidebar-accent text-white shadow-[0_0_12px_rgba(37,99,235,0.3)]'
              : 'text-sidebar-foreground/50 hover:text-sidebar-foreground hover:bg-white/[0.08]'
          )}
          aria-label={item.label}
          aria-current={isActive ? 'page' : undefined}
        >
          {isActive && (
            <span className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-[2px] w-[3px] h-4 rounded-r-full bg-sidebar-accent" />
          )}
