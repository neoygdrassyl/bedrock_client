...
function RECORD_LAW_PDF(props) {
    const { translation, swaMsg, globals, currentItem, currentVersion, currentRecord, currentVersionR, noReport } = props;

    const _GET_CHILD_REVIEW_CLASS = () => {
    ...
    const _GET_CHILD_RECORD_REVIEW = () => {
        var _CHILD = props.currentRecord.record_law_reviews;
        var _CURRENT_VERSION = document.getElementById('record_version').value;
        var _CHILD_VARS = {
            id: "",
            worker_id: "",
    ...
    const LOAD_STEP = (_id_public) => {
        let _currentRecord = props.currentRecord
        var _CHILD = _currentRecord.record_law_steps || [];
        for (var i = 0; i < _CHILD.length; i++) {
            if (_CHILD[i].version == document.getElementById('record_version').value && _CHILD[i].id_public == _id_public) return _CHILD[i]
        }
        return []
    }
    const _GET_STEP_TYPE = (_id_public, _type) => {
    ...
    const CREATE_CHECK = async (_detail, chekcs, _currentItem, _headers, _date) => {
        ...
        const id_public = _item.id_public;
    ...
    let CREATE_PDF_CHECK = () => {
        ...
        let _detail = _GET_CHILD_RECORD_REVIEW().detail;
        let _details = '';
        ...
        let checks = [];
        let _child = _GET_CHILD_REVIEW_CLASS();
