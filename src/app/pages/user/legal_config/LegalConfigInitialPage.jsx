...
function LabelManagerModal({ open, labels, documents, onClose, onUpdate }) {
  const [query, setQuery] = useState('');
  ...
  const [error, setError] = useState('');
  const visible = labels.filter((label) => label.name.toLocaleLowerCase().includes(query.toLocaleLowerCase()));
  async function deactivate() {
    if (!candidate || busyId) return;
    setBusyId(candidate.id); setError('');
    try { await onUpdate(candidate, { is_active: false }); setCandidate(null); } catch (err) { setError(extractErrorMessage(err, 'No fue posible desactivar la etiqueta. Intenta nuevamente.')); } finally { setBusyId(''); }
  }
  async function saveEdit() {
    if (!editing || busyId || !draftName.trim()) return;
    setBusyId(editing.id); setError('');
    try { await onUpdate(editing, { name: draftName.trim(), slug: slugify(draftName) }); setEditing(null); } catch (err) { setError(extractErrorMessage(err, 'No fue posible renombrar la etiqueta. Intenta nuevamente.')); } finally { setBusyId(''); }
  }
  return <><Dialog open={open} onOpenChange={(next) => { if (!next && !busyId) onClose(); }}><DialogContent className="legal-config-detail-dialog"><DialogHeader><DialogTitle>Etiquetas</DialogTitle><DialogDescription>Administra etiquetas reutilizables sin borrar sus vínculos históricos.</DialogDescription></DialogHeader>{error && <DovelaInlineAlert tone="danger">{error}</DovelaInlineAlert>}<label className="visually-hidden" htmlFor="label-manager-search">Buscar etiqueta</label><input id="label-manager-search" className="legal-config-native-select" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar etiqueta" /><DovelaDataTable ariaLabel="Etiquetas documentales" emptyMessage="No hay etiquetas." rows={visible.map((label) => ({ ...label, key: label.id }))} columns={[{ key: 'name', header: 'Etiqueta', render: (label) => <strong>{label.name}</strong> }, { key: 'label_scope', header: 'Ámbito' }, { key: 'is_active', header: 'Estado', render: (label) => label.is_active === false ? 'Desactivada' : 'Activa' }, { key: 'documents', header: 'Documentos', render: (label) => { const names = documents.filter((document) => (document.label_ids || []).includes(label.id)).map((document) => document.name); return <details><summary>{names.length}</summary>{names.join(', ') || 'Sin documentos'}</details>; } }, { key: 'actions', header: 'Acciones', render: (label) => <div className="legal-config-definition-row-actions"><DovelaButton tone="neutral" size="sm" disabled={Boolean(busyId)} onClick={() => { setError(''); setEditing(label); setDraftName(label.name); }}>Editar</DovelaButton><DovelaButton tone="danger" size="sm" disabled={label.is_active === false || Boolean(busyId)} onClick={() => { setError(''); setCandidate(label); }}>Desactivar</DovelaButton></div> }]} /><DialogFooter><DovelaButton tone="neutral" onClick={onClose} disabled={Boolean(busyId)}>Cerrar</DovelaButton></DialogFooter></DialogContent></Dialog><Dialog open={Boolean(candidate)} onOpenChange={(next) => { if (!next && !busyId) setCandidate(null); }}><DialogContent><DialogHeader><DialogTitle>Desactivar etiqueta</DialogTitle><DialogDescription>Las asociaciones históricas se conservarán.</DialogDescription></DialogHeader>{error && <DovelaInlineAlert tone="danger">{error}</DovelaInlineAlert>}<DialogFooter><DovelaButton tone="neutral" disabled={Boolean(busyId)} onClick={() => setCandidate(null)}>Cancelar</DovelaButton><DovelaButton tone="danger" loading={Boolean(busyId)} onClick={deactivate}>Desactivar etiqueta</DovelaButton></DialogFooter></DialogContent></Dialog><Dialog open={Boolean(editing)} onOpenChange={(next) => { if (!next && !busyId) setEditing(null); }}><DialogContent><DialogHeader><DialogTitle>Editar etiqueta</DialogTitle></DialogHeader>{error && <DovelaInlineAlert tone="danger">{error}</DovelaInlineAlert>}<DovelaField label="Nombre"><DovelaInput value={draftName} onChange={(event) => setDraftName(event.target.value)} autoFocus /></DovelaField><DialogFooter><DovelaButton tone="neutral" disabled={Boolean(busyId)} onClick={() => setEditing(null)}>Cancelar</DovelaButton><DovelaButton tone="primary" loading={Boolean(busyId)} onClick={saveEdit}>Guardar cambios</DovelaButton></DialogFooter></DialogContent></Dialog></>;
}
...
function TypologyManagerModal({ open, typologies, onClose, onCreate, onUpdate }) {
  ...
  async function save() {
    ...
    try {
      if (draft.id) await onUpdate(draft, { name: draft.name.trim(), description: draft.description?.trim() || null, is_active: draft.is_active !== false });
      else await onCreate({ code: draft.code.trim().toUpperCase(), name: draft.name.trim(), description: draft.description?.trim() || null });
      setDraft(null);
    } catch (err) { setError(extractErrorMessage(err, 'No fue posible guardar la tipología. Intenta nuevamente.')); } finally { setBusy(false); }
  }
  async function deactivate() { if (!candidate) return; setBusy(true); setError(''); try { await onUpdate(candidate, { is_active: false }); setCandidate(null); } catch (err) { setError(extractErrorMessage(err, 'No fue posible desactivar la tipología. Intenta nuevamente.')); } finally { setBusy(false); } }
  const previewCode = draft?.code?.trim().toUpperCase() || 'PLAN-';
  const previewName = draft?.name?.trim() || 'Nombre de la tipología';
  return <><Dialog open={open} onOpenChange={(next) => { if (!next && !busy) onClose(); }}><DialogContent className="legal-config-detail-dialog"><DialogHeader><DialogTitle>Tipologías documentales</DialogTitle><DialogDescription>Administra tipologías reutilizables sin afectar documentos históricos.</DialogDescription></DialogHeader>{error && <div aria-live="assertive"><DovelaInlineAlert tone="danger">{error}</DovelaInlineAlert></div>}<div className="legal-config-manager-toolbar"><DovelaInput value={query} onChange={(event) => setQuery(event.target.value)} aria-label="Buscar tipología" placeholder="Buscar tipología" /><DovelaButton tone="primary" onClick={() => { setError(''); setDraft({ code: '', name: '', description: '' }); }}>Nueva tipología</DovelaButton></div><DovelaDataTable ariaLabel="Tipologías documentales" emptyMessage="No hay tipologías. Crea una para usar documentos con prefijo." rows={visible.map((item) => ({ ...item, key: item.id }))} columns={[{ key: 'code', header: 'Prefijo' }, { key: 'name', header: 'Tipología', render: (item) => <div><strong>{item.name}</strong><small>{item.description || 'Sin descripción'}</small></div> }, { key: 'is_active', header: 'Estado', render: (item) => item.is_active === false ? 'Desactivada' : 'Activa' }, { key: 'actions', header: 'Acciones', render: (item) => <div className="legal-config-definition-row-actions"><DovelaButton tone="neutral" size="sm" onClick={() => setDraft(item)}>Editar</DovelaButton><DovelaButton tone="danger" size="sm" disabled={item.is_active === false} onClick={() => setCandidate(item)}>Desactivar</DovelaButton></div> }]} /><DialogFooter><DovelaButton tone="neutral" onClick={onClose} disabled={busy}>Cerrar</DovelaButton></DialogFooter></DialogContent></Dialog><Dialog open={Boolean(draft)} onOpenChange={(next) => { if (!next && !busy) setDraft(null); }}><DialogContent className="legal-config-creation-dialog"><DialogHeader className="legal-config-creation-hero"><span className="legal-config-creation-eyebrow">Catálogo documental</span><DialogTitle>{draft?.id ? 'Editar tipología' : 'Nueva tipología'}</DialogTitle><DialogDescription>{draft?.id ? 'Actualice la información descriptiva sin modificar el prefijo histórico.' : 'Defina una clasificación reutilizable para los documentos del catálogo.'}</DialogDescription></DialogHeader><div className="legal-config-modal-form legal-config-creation-form"><div className="legal-config-creation-grid"><DovelaField label="Código o prefijo" required><DovelaInput aria-label="Código o prefijo" value={draft?.code || ''} onChange={(event) => setDraft((current) => ({ ...current, code: event.target.value.toUpperCase() }))} placeholder="Ej. PLAN-" disabled={Boolean(draft?.id)} autoFocus required /><small className="legal-config-field-help">Se conserva como parte de la identificación documental.</small></DovelaField><DovelaField label="Nombre de la tipología" required><DovelaInput aria-label="Nombre de la tipología" value={draft?.name || ''} onChange={(event) => setDraft((current) => ({ ...current, name: event.target.value }))} placeholder="Ej. Planos" required /></DovelaField></div><DovelaField label="Descripción"><textarea className="legal-config-textarea" value={draft?.description || ''} onChange={(event) => setDraft((current) => ({ ...current, description: event.target.value }))} placeholder="Explique cuándo conviene usar esta tipología." rows={3} /></DovelaField><section className="legal-config-creation-preview" aria-label="Vista previa de la tipología"><span>Vista previa</span><div className="legal-config-typology-preview"><code>{previewCode}001</code><div><strong>{previewName}</strong><small>Así se verá en la clasificación de un documento.</small></div></div></section><DialogFooter><DovelaButton tone="neutral" onClick={() => setDraft(null)} disabled={busy}>Cancelar</DovelaButton><DovelaButton tone="primary" onClick={save} loading={busy} disabled={!draft?.code?.trim() || !draft?.name?.trim()}>Guardar tipología</DovelaButton></DialogFooter></div></DialogContent></Dialog><Dialog open={Boolean(candidate)} onOpenChange={(next) => { if (!next && !busy) setCandidate(null); }}><DialogContent><DialogHeader><DialogTitle>Desactivar tipología</DialogTitle><DialogDescription>La tipología dejará de estar disponible para nuevos documentos. Los vínculos históricos se conservarán.</DialogDescription></DialogHeader><DialogFooter><DovelaButton tone="neutral" onClick={() => setCandidate(null)} disabled={busy}>Cancelar</DovelaButton><DovelaButton tone="danger" onClick={deactivate} loading={busy}>Desactivar tipología</DovelaButton></DialogFooter></DialogContent></Dialog></>;
}
...
export default function LegalConfigInitialPage() {
  ...
  async function loadData() {
    ...
    setErrorMessage('');
    try {
      const [actuations, documents, labels, texts, contracts, assertions, rules, nextTypologies] = await Promise.all([
  ...
  return (
    ...
    <section className="legal-config-page legal-config-page--simple" data-testid="legal-config-page">
      ...
      <DocumentDefinitionModal
        ...
        onRequestCreateLabel={(name) => setLabelNameToCreate(name)}
